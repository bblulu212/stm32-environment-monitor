...
{
        RCC_APB2PeriphClockCmd (RCC_MQ_GPIO_AO, ENABLE); // 使能时钟
        RCC_APB2PeriphClockCmd (RCC_MQ_GPIO_DO, ENABLE); // 使能时钟	
        RCC_APB2PeriphClockCmd (RCC_MQ_ADC, ENABLE); // 使能ADC时钟

        GPIO_InitTypeDef  GPIO_InitStructure; // GPIO配置结构体

        GPIO_InitStructure.GPIO_Pin = GPIO_MQ_AO;
        GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;    //模拟输入

        GPIO_Init(PORT_MQ_GPIO_AO, &GPIO_InitStructure); // 初始化GPIO

        GPIO_InitStructure.GPIO_Pin = GPIO_MQ_DO;
        GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;    //输入

        GPIO_Init(PORT_MQ_GPIO_DO, &GPIO_InitStructure); // 初始化GPIO
...
/******************************************************************
 * 函 数 名 称：Get_MQ7_DO_value
 * 函 数 说 明：获取MQ7DO引脚的电平状态
 * 函 数 形 参：无
...
{
    if( GPIO_ReadInputDataBit(PORT_MQ_GPIO_DO, GPIO_MQ_DO) == RESET )
    {
        return 0;
    }
    else
